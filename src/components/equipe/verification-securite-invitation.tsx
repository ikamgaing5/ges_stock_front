...
export function VerificationSecuriteInvitation({
  ouvert = true,
  onFermer,
  onSucces,
  mode = "modal",
  ...
  async function demanderCode() {
    ...
    try {
      ...
      }>("/employes/invitations/demander-code");

      setCodeDemande(true);
      setCompteARebours(res.delai_renvoi_secondes || 60);
      toast.success(res.message || t("equipe.codeEnvoyeA", { email: res.email }));
    } catch (e) {
  ...
  async function valider(evenement?: React.FormEvent) {
    ...
    try {
      ...
      }>("/employes/invitations/verifier-acces", payload);

      // Mémorisation du jeton de vérification
      if (typeof window !== "undefined") {
        window.sessionStorage.setItem("telora_invitation_token", res.token_verification);
      }

      toast.success(res.message || t("equipe.accesDeverrouille"));
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2 rounded-xl bg-muted/60 p-1.5 border border-border/40">
        ...
        >
          ...
          <span>{t("equipe.methodeMotDePasse")}</span>
        </button>
        <button
          type="button"
          onClick={() => {
      ...
      {methode === "mot_de_passe" ? (
        <form onSubmit={valider} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="verif-mot-de-passe">
              {t("equipe.motDePasseLabel")}
            </Label>
            <div className="relative">
              <Input
                id="verif-mot-de-passe"
                type={afficherMdp ? "text" : "password"}
                placeholder={t("equipe.motDePassePlaceholder")}
                value={motDePasse}
                onChange={(e) => {
            ...
            <p className="text-xs text-muted-foreground">
              Entrez le mot de passe de votre compte ({utilisateur?.email}) pour confirmer que vous êtes bien la propriétaire.
            </p>
          </div>
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3.5 text-xs text-muted-foreground space-y-1">
                <div className="flex items-center gap-1.5 font-medium text-foreground">
                  ...
                  <span>{t("equipe.codeEnvoyeA", { email: utilisateur?.email ?? "" })}</span>
                </div>
                <p className="text-[11px] text-muted-foreground">
                  {t("equipe.infoValidite15Min")}
                </p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    <span>{t("equipe.retourVueEnvoi")}</span>
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  >
                    ...
                    {compteARebours > 0
                      ? t("equipe.renvoyerCodeDans", { secondes: compteARebours })
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-border/60 bg-muted/30 p-4 space-y-3">
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                  ...
                  <span>{t("equipe.infoValidite15Min")}</span>
                </div>
              </div>
              ...
              >
                ...
                <span>{t("equipe.envoyerCodeBtn")}</span>
              </Button>
            </div>
          )}
        </div>
      ...
      <Card className="mx-auto max-w-lg border-border/80 shadow-md">
        <CardContent className="p-6 sm:p-8">
          <div className="mb-6 flex flex-col items-center text-center">
            {/* <div className="mb-3.5 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-primary">
              <ShieldCheck className="h-7 w-7" />
            </div> */}
            <h3 className="font-heading text-lg font-bold text-foreground">
              {t("equipe.verificationRequiseTitre")}
            </h3>
            <p className="mt-1 text-sm text-muted-foreground">
              {t("equipe.verificationRequiseDesc")}
            </p>
          </div>
